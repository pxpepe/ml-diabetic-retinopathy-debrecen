...
def acc_sample_dim(classifier, X, X_gtest, y, y_gtest):
    ...
    for i in np.arange(0.95, 0.14, -0.1):
        ...
        X_train, X_test, y_train, y_test = split_dataset(X, y, test_size=i)
...
def print_binary_roc_curve(y_test, y_probas,class_proba=1):
    from sklearn.metrics import roc_curve, auc
    import matplotlib.pyplot as plt
    
    fpr, tpr, _ = roc_curve(y_test, y_probas[:,class_proba])
    roc_auc = auc(fpr, tpr)
    
    plt.figure()
    plt.plot(fpr, tpr, color='darkorange',
             lw=2, label='ROC (área = %0.2f)' % roc_auc)
